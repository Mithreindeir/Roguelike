...
void generate_map(struct game_map *map)
{
	...
	fill_rect(map, map->w / 2 - size / 2, map->h / 2 - size / 4, size,
		  size / 2);
	...
	while (success > 0 && max_tries > 0) {
		...
		if (os != success) {
			if (success == 0) {
				add_obj(map, x + w / 2, y + h / 2, '^', END,
					NULL, NULL);
			} else if (random_max(10) > 3) {
				int ix =
				    add_obj(map, x + w / 2, y + h / 2, 's',
					    ENEMY, &enemy_update,
					    (game_object_free) & destroy_enemy);
	...
	for (int i = 0; i < map->h; i++) {
		for (int j = 0; j < map->w; j++) {
			map->nodes[j + (i * map->w)] =
			    node_init(j, i, map->map[j + i * map->w] == WALL);
		}
	}
	//if (max_tries <= 0)
	//      generate_map(map);
