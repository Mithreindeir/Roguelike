...
void enemy_update(struct game_map *map, struct game_object *obj, int ch)
{
	...
	if (newa) {
		int idx0 =
		    map->objects[pidx]->x + map->objects[pidx]->y * map->w,
		    idx1 = obj->x + obj->y * map->w;

		a_star(map->nodes[idx0], map->nodes[idx1], map,
		       (struct node_stack *
			(*)(void *, struct node *))get_neighbors);
