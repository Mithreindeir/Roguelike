...
void game_loop()
{
	...
	generate_map(map);
	int pi =
	    add_obj(map, map->w / 2, map->h / 2, '@', PLAYER, &player_update,
		    (game_object_free) & destroy_player);
	...
	while (1) {
		...
		float sway =
		    round(sin(round(clock() / (CLOCKS_PER_SEC / 2)) * 2));
		...
		if (scanning) {
			scan(map, map->objects[pi]->x, map->objects[pi]->y, 6,
			     iter);
		...
		for (int i = 0; i < map->num_objects; i++) {
			if (map->seen[map->objects[i]->y * map->w +
				      map->objects[i]->x]
			    && !map->objects[i]->passive)
				mvaddch(map->objects[i]->y,
					map->objects[i]->x + map->offset,
					map->objects[i]->ch);
		}
		attron(COLOR_PAIR(1));
		mvprintw(5, 1, "%s", "Menu:");
